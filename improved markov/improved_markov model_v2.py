...
def markov_pred(juzhen1,juzhen2,d1,t1,t2,bjs):
    ...
    if bj==0:
        ...
        if np.sum(td2[t2,:])>0:
            pred1=np.argsort(td2[t2,:])[::-1][0:10].tolist()
        else:
            pred1=np.argsort(od[d1,:])[::-1][0:10].tolist()
    
    elif bj==3:
        td1=np.sum(juzhen1,axis=0).transpose()
        if np.sum(td1[t1,:])>0:
            pred1=np.argsort(td1[t1,:])[::-1][0:10].tolist()
        else:
            pred1=np.argsort(od[d1,:])[::-1][0:10].tolist()
    
    elif bj==4:
        if np.sum(juzhen2[d1,:,t2])>0:
            pred1=np.argsort(juzhen2[d1,:,t2])[::-1][0:10].tolist()
        else:
            pred1=np.argsort(od[d1,:])[::-1][0:10].tolist()

    else:
        if np.sum(juzhen1[d1,:,t1])>0:
            pred1=np.argsort(juzhen1[d1,:,t1])[::-1][0:10].tolist()
        else:
            pred1=np.argsort(od[d1,:])[::-1][0:10].tolist()
...
def han1(mm):
    ...
    a,b,c,d,e,f=mm
    nn=([d,c,f,b,e,a])#d,od,td,td,tod,tod
    nn=[round(m,3) for m in nn]
    rr=max(nn)
    nn=np.array(nn)
...
df2[300]=df2[[2,3,4,5,6,7]].apply(func7,axis=1)#rfce

outname1='./'+flag+'_improved_markov.txt'
...
for ij in ff.readlines():
    ...
    print(user)
    if 'test' in binds:
        tmess=df2.loc[(df2[0]==user)].iloc[0,:].tolist()
        dseq1=mess[1].split('&')
        tseq1=mess[2].split('&')
        ...
        for j in range(1,len(dseq11)):
            ...
            if binds[j]=='test':
                numa=numa+1
                pred=markov_pred(juzhen1,juzhen2,d1,t2,t3,tmess[-1])
                if d2 in pred:
                    ind=pred.index(d2)
                    if ind==0:
                        top1=top1+1
